...
4. ~~Prompt copy for the opt-in choice, and whether the UI pre-selects either option.~~ **Resolved, built 2026-09-04: no pre-selection anywhere.** Two surfaces ended up needing two different shapes of "no pre-selection", not one: the claim-review screen (§Phase 3) asks with two equal, both-`required` radio buttons inside the claim form the person is already completing as one deliberate act — matching this section's own reasoning that a pre-selected "share" is a dark-pattern consent gate. The one-time sign-in prompt for pre-existing accounts (`/roster-visibility`) is a courtesy interruption, not a form the person came to fill out, so it follows onboarding's own "every step can be skipped, and skipping still counts as finishing" rule instead: the same two equal buttons, plus a third, separately-submitted "Not now" that answers the prompt (stamps `roster_visibility_chosen_at`) without answering the question — `roster_visibility` stays whatever it already was (null/hidden for everyone who reaches this screen, see §9's Phase 2/3 entry below for why that is always true here).

## 9a. Build log — Phase 2/3 (types, service layer, UI) built 2026-09-04
