...
**Built and applied:** the database schema and Row Level Security policies (`supabase/migrations/`, mirrored as Zod schemas in `packages/types/src/db/`), **the legacy data import**, and — also as of 2026-08-13 — **the Kinde → Supabase auth bridge on web** (`apps/web/src/server/`). API routes and the connection-verification logic are still to build.
...
- **Two secrets are still needed** before the flow can be exercised over real HTTP: `SUPABASE_SERVICE_ROLE_KEY` and `SUPABASE_JWT_SECRET`, both from the Supabase dashboard. `.env.local` says exactly where. Until they are set, `/auth-check` reports which one is missing instead of failing obscurely.

**Next up, in order:** finish the auth bridge run end-to-end once those two secrets are in place (`pnpm --filter web dev`, then visit `/auth-check`), then the photo backfill, then Phase 1 features, which still need their own schema and RLS work built on top of what is already applied: Profile, then the Connect Flow (§4). Mobile Kinde wiring is deliberately not started — it needs the EAS development build (§7.2) and the web proof should be reviewed first.
